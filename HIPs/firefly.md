...
A key design principle for Hyperledger FireFly is to be a sort of "umbrella system" - to enable a set of complementary technologies to be used together via a simple API. For example, consortia often need different levels of privacy in data exchange and often leverage different types of compute to execute business logic based upon business requirements.  FireFly therefore intends to integrate in popular technologies that consortia use together.  A simple plugin interface was designed to wrap these technologies.  Examples include:
...
A key aspect of the FireFly system is it's straightforward REST API.   For example, an operation that we've found very common in enterprise use cases is to send data org to org off-chain.  Pinning both the data itself on chain, the send operation, and even the delivery receipt are valuable operations that leverage the blockchain as a sort of audit log. Having a simple "send" API in FireFly to accomplish the task means developers don't have to "re-invent the wheel", which can be a considerable engineering challenge as it requires much of the same logic and components that FireFly offers.

The FireFly API provides building blocks for several such common patterns.  From an application development perspective, this becomes very simple to integrate into the application.  While these patterns cover common scenarios, they of course do not cover all scenarios or operations.  When custom on-chain smart contract logic is required, the API allows it to be invoked via the same event-driven programming paradigm at the application level.

Blockchain protocol API's or SDK's are powerful but understandably very specific to the blockchain system itself.  This requires a high degree of specialized knowledge in order to build even simple use cases.  For example, in Ethereum the developer must RLP encode and handle nonce management herself, which requires knowledge about pending transaction pools, failure and retry, and more.  In order to process events from on-chain activities, again a large amount of event process logic is needed.  

FireFly provides pre-built components with customizeable to simplify the job for the application developer.  Developers build to this convenient REST API using their existing API/web/mobile development skill-set. They scale their application using normal n-way cluster scale approaches, and can think about the multi-party system tier in the same way they think about their other infrastructure components, like databases and messaging systems.
